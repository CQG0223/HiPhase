...
def train(args, model, enc=False):
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            inputs = Variable(images)   #images: [4,1,1024,1024]
            targets = Variable(labels)  #labels: [4,1,128,128]

            optimizer.zero_grad()
            with autocast():
                outputs = model(inputs, only_encode=enc)
                loss = loss_func(outputs, targets[:, 0])    #outputs: [4,18,128,128] targets:[4,1,128,128]
                logger.log(losses={'train_loss':loss.item()},
                                images={'imgae1':images[0,:,:,:],'GT':targets[0,:,:,:]})
...
def main(args):
    ...
    model = Net(NUM_CLASSES, encoder=Enc)  # Add decoder to encoder
    if args.cuda:
        model = torch.nn.DataParallel(model).cuda()
